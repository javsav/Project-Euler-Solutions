...
for a in range(triangle_height, 0, -1): # Declaring var a for algorithm. Range will start the height of the algorithm at tri[14], with a step of negative 1 in order to move from the bottom to the top of the triangle
    ...
    for b in range(row_length): # Declaring var b for algorithm, range is row_length as discussed above.
        ...
        if current_value > next_value: # Whichever of "current_value" and "next_value" is larger will always create the biggest sum because the "shared_value" is constant for each triplet.
			...
			new_array.append(next_value + shared_value) # Largest of each triplet in the two rows is appended to temporary new array. First example - 62+63 = 125
			
               
    tri.pop() #Removes one row of the triangle - first e.g. tri[14]
	
    tri.pop() #Removes the next row of the triangle - first e.g tri[13]
